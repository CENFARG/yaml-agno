...
#                Verified against Agno v2.6.22: MemoryManager coerces user_id=None
...
async def recall_on_start(agent,
    ...
    if learning_cfg.enabled:
        ...
        # See agno/learn/machine.py (Agno v2.6.22).
    ...
    # See agno/memory/manager.py (Agno v2.6.22).
...
#                the real Agno v2.6.22 APIs directly.
...
class AutosaveManager:
    ...
    based on the learning.* flags. The write target is selected as follows:
      ...
      * learning.enabled is False -> ``MemoryManager.add_user_memory(memory=UserMemory(...), user_id)``
        ...
        v2.6.22 (no async variant), so it is called WITHOUT await.
    """
    ...
    async def on_agent_decision(self, agent_name: str, decision: str,
        ...
        if self.learning_cfg.enabled:
            ...
            # decision_log.py and agno/learn/schemas.py (Agno v2.6.22).
        ...
        # v2.6.22 (no async variant). Call it without await.
    ...
    async def on_discovery(self, title: str, content: str,
        ...
        if self.learning_cfg.enabled:
            ...
            # See agno/learn/stores/learned_knowledge.py (Agno v2.6.22).
...
- **RED**:
  ...
  async def test_autosave_decision_routes_to_user_memory(fake_agent_simple):
      ...
      # MemoryManager.add_user_memory is synchronous in Agno v2.6.22.
  ```
