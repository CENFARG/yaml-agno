...
> @ai-directive (naming SSOT): the detection method is `is_native_reasoning_model()`. An earlier draft referenced `detect_native_reasoning()` — that method does NOT exist in Agno v2.6.22 and must not appear in yaml-agno code or docs.
