...
> `continued` es un evento/transitorio; el estado estacionario post-resume vuelve a `running`. `regenerated` (Agno 2.8.3, `agno/run/base.py:337`) marca un run regenerado que coexiste junto al original; **NO es terminal** — yaml-agno no lo incluye en `TERMINAL` y lo trata como variante observacional de `running`/`completed`.
...
# Members (Agno 2.8.3, run/base.py:323-338):
