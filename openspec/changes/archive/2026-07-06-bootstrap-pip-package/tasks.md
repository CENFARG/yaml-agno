...
- [x] 1.2 Crear `pyproject.toml` con el contenido modelo del design §pyproject.toml model (hatchling, src-layout, deps `agno==2.6.22`, `core-cenf-py @ git+...@v0.1.0`, `pydantic>=2.0`, `pyyaml>=6.0`, extras `dev`, ruff/mypy/pytest config). **Req: Dependencias pinneadas / Instalabilidad editable / Versión SSOT / Lint / Types.**
- [x] 1.3 Crear `src/yaml_agno/__init__.py` con el bloque `__version__` vía `importlib.metadata.version("yaml-agno")` (design §src/yaml_agno tree). **Req: Versión SSOT.**

## Phase 2: Namespaces canónicos vacíos

- [x] 2.1 Crear los **21** `src/yaml_agno/{sub}/__init__.py` vacíos (0 bytes):
...
- [x] 3.1 Escribir `tests/test_smoke.py` (2 tests del design: `test_package_version_matches_pyproject`, `test_package_is_importable`). **Req: Smoke test day-1 green.**
- [x] 3.2 Extender `.gitignore`: tras diff contra el actual, añadir SOLO `*.whl`, `src/*.egg-info/`, `src/yaml_agno.egg-info/` (sin duplicar lo ya presente). **Req implícito (higiene build artifacts).**
- [x] 3.3 Crear `.github/workflows/ci.yml`: matrix `[3.12, 3.13]`, `cache: pip`, pasos `ruff check` + `mypy src/yaml_agno` + `pytest`. **INCLUIR** el paso de inyección de credenciales para el dep privado: `git config --global url."https://x-access-token:${{ secrets.GIT_AUTH_TOKEN }}@github.com/".insteadOf "https://github.com/"` ANTES de `pip install -e ".[dev]"`. Documentar en comentario del YAML que sin el secret el job falla. **Req: CI workflow válido y matriz.**
- [x] 3.4 Crear `README.md` mínimo (1 párrafo: qué es yaml-agno + cómo `pip install -e ".[dev]"`). **Req implícito (readme = PEP 621 field).**

## Phase 4: Verificación (GREEN estructural)

- [x] 4.1 `pip install -e ".[dev]"` verde (Python 3.12 y 3.13). **Scenario: Instalación editable 3.12 / 3.13.**
- [x] 4.2 `python -c "import yaml_agno; print(yaml_agno.__version__)"` imprime `0.1.0`. **Scenario: Versión leída desde metadata.**
- [x] 4.3 Import de los 21 sub-paquetes sin error. **Scenario: Todos los sub-paquetes son importables.**
- [x] 4.4 `python -m pytest` verde (2/2), `pytest-asyncio` cargando. **Scenario: Pytest verde.**
- [x] 4.5 `ruff check .` sin findings (F401 ok en `__init__.py`). **Scenario: Ruff sin findings.**
- [x] 4.6 `mypy src/yaml_agno` sin errores. **Scenario: Mypy sin errores.**
- [x] 4.7 `git diff --name-only specs/` vacío (ningún SPEC editado). **Scenario: specs/ sin cambios.**

## Phase 5: Commit granular (conventional commits)

- [x] 5.1 Commits atómicos en orden: `chore: add pyproject.toml and package root` (1.2+1.3) → `chore: add 21 canonical empty namespaces` (2.1) → `test: add package smoke test` (3.1) → `chore: extend gitignore for build artifacts` (3.2) → `ci: add github actions workflow with private dep auth` (3.3) → `docs: add minimal README` (3.4). **No commitear specs/.**
