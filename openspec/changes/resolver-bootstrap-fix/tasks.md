...
- [x] 0.1 Confirm clean baseline: `python -m pytest -m "unit and not integration"
...
- [x] 1.1 Add `test_build_agno_resolver_seeds_defaults_when_empty` to
...
- [x] 1.2 Add `test_build_agno_resolver_strict_preserves_crash`: same setup,
  call with `strict_allowlist=True`, assert `ValidationError` raised.
  **Spec**: Req 1 Scenario 1.2.
- [x] 1.3 Add `test_build_agno_resolver_honors_explicit_allowlist`: seed
...
- [x] 1.4 Add `AGNO_ALLOWLIST_PREFIXES` to the existing import block in
...
- [x] 2.1 Add `test_config_secret_resolver_falls_back_to_environ` to
...
- [x] 2.2 Add `test_config_secret_resolver_config_wins_over_environ`: config
...
- [x] 2.3 Add `test_config_secret_resolver_both_missing_returns_none`:
  `monkeypatch.delenv(..., raising=False)`, empty config, assert None.
  **Spec**: Req 4 Scenario 4.3.

### GREEN

- [x] 2.4 Add `import os` to `secret_resolver.py`. Rewrite
...
- [x] 3.1 Add `test_provider_factory_raises_on_missing_api_key_cloud` to
...
- [x] 3.2 Add `test_provider_factory_local_provider_no_raise_on_missing_key`:
...
- [x] 3.3 Extend `ModelConstructionError.__init__` with `env_name:
...
- [x] 4.1 Add `test_build_with_provider_factory_uses_model_instance` to
...
- [x] 4.2 Add `test_build_without_provider_factory_keeps_string_passthru`:
...
- [x] 4.3 Add imports (`ProviderFactory`, `ModelExpandedSpec`,
...
- [x] 5.1 `python -m pytest -m "unit and not integration" -q` → all green
  (400 baseline + new tests).
- [x] 5.2 `ruff check .` → clean.
- [x] 5.3 `mypy src/yaml_agno` → no new errors vs. baseline.
- [x] 5.4 Integration smoke (manual or new `@pytest.mark.integration` test):
