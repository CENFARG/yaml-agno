...
| VQ | Decisión | Check | Expected |
|----|----------|-------|----------|
| VQ010 | D-F1-02 JWT native always-on + isolation refusal | `_require_isolated_auth`: `user_isolation is not True` con `authorization=True` → `ValueError` (YamlAgentOS, antes de `super().__init__`) / `RuntimeError` (run_server, antes de `create_app`); `is True` estricto | aislado o rechazado; dev path intacto |
...
- D-F1-10 (2026-08-18): **R1 NO estaba resuelto — reproducido en Agno 2.8.7 instalado**: `AuthorizationConfig` acepta exactamente 7 campos (`verification_keys, jwks_file, algorithm, verify_audience, audience, admin_scope, user_isolation`); `basic_auth` y keys desconocidas de `config` se DROPEAN silenciosamente (pydantic extra=ignore). `AuthorizationAdapter.build()` hacía `kwargs.update()` ciego → no-op/fail-open declarativo vivo. **Fix = S5a.0 (prerrequisito de S5a)**: whitelist explícito de los 7 campos + fail-fast `AuthorizationBuildError` en keys desconocidas/basic_auth + tests de contrato que asierten el CONTENIDO del AuthorizationConfig construido. Ver VQ011. **Extensión 2026-09-01 (auth-vq010-isolation-guard, cierra Judgment Day CRITICAL #3120)**: Agno 2.8.7 defaultea `user_isolation=False`, así que `authorization=True` sin config aislada booteaba un servidor autenticado-pero-no-aislado. Nuevo predicado privado `_require_isolated_auth` en `agentos/authorization_adapter.py` (dispara si `authorization is True` Y (`authorization_config is None` O `user_isolation is not True`); identidad estricta, `"true"`/`1` no pasan): `YamlAgentOS.__init__` lanza `ValueError` ANTES de `super().__init__` (tras el bloque JD-01, que queda intacto) y `run_server` lanza `RuntimeError` ANTES de `create_app` (guard viejo `is not True` intacto). Camino dev (`authorization=False`) sin cambios. VQ010 ajustado de grep de presencia a redacción de rechazo (ver `.chats/decisions.yaml`). Ruta de fábrica sin cambios: el adaptador sigue construyendo `user_isolation=True` (invariante VQ011, test-only).
