"""Unit tests for TenantContextMiddleware (SPEC_06 §3.2 dev-only contract, S5a.1 WU3).

Covers the composite user_id resolution contract for dev / non-JWT mode:
  ...
  4. Missing tenant_id causes UserIdentityResolutionError (401 response).
  5. user_id is never None when tenant_id + memory_cfg are valid.

In production JWT mode (authorization=True), TenantContextMiddleware is not mounted
(JD-01 structural exclusion); Agno's native AuthMiddleware handles identity.
"""
...
class TestTenantContextMiddlewareFailFast:
    ...
    def test_missing_tenant_id_returns_401(self) -> None:
        """No X-Tenant-Id → resolve_user_id raises.
...
class TestTenantContextMiddlewareDelegationContract:
    ...
    def test_passes_correct_args_to_resolve_user_id(self, mock_resolve) -> None:
        ...
        - principal_id is None (dev header mode).
