...
def test_base_is_declarative_and_import_is_side_effect_free(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import yaml_agno.db.base as db_base  # noqa: F811  (re-import after spy)
...
def test_timestamp_and_tenant_mixins_define_columns() -> None:
    """Throwaway record using both mixins exposes created_at/updated_at/
    tenant_id with correct nullability and CASCADE on the FK."""
    from yaml_agno.db.base import Base, TenantMixin, TimestampMixin
    from sqlalchemy.orm import Mapped, mapped_column
    from sqlalchemy import String

    class _Throwaway(TimestampMixin, TenantMixin, Base):
    ...
    cols = {c.name: c for c in _Throwaway.__table__.columns}
    ...
    Base.metadata.remove(_Throwaway.__table__)
...
def test_metadata_attribute_maps_to_metadata_column() -> None:
    ...
    table = AgentRecord_table = AgentConfigRecord.__table__
...
def test_tenant_round_trip_in_sqlite() -> None:
    ...
    tenant_id = uuid4()
    settings_payload = {"theme": "dark", "limits": {"qps": 100}}
    insert_sql = text(
        """
        INSERT INTO "yamlagno"."yamlagno_tenants"
    ...
    with engine.begin() as conn:
        conn.execute(
            insert_sql,
            {
                ...
                "name": "Acme",
                "slug": "acme",
                "tenant_kind": "acme-default",  # invalid; overridden below
                "parent_org_id": None,
                "settings": '{"theme": "dark", "limits": {"qps": 100}}',
            },
        )

    # The CHECK constraint valid_tenant_kind MUST reject the bad value.
    # SQLite enforces CHECKs at insert time, so the previous insert should
    # already have failed; wrap the whole scenario in a fresh engine to be
    # safe.
    ...
    with engine2.begin() as conn:
        conn.execute(
            text(
                ...
                INSERT INTO "yamlagno"."yamlagno_tenants"
                    (id, name, slug, tenant_kind, parent_org_id, settings)
                VALUES (:id, :name, :slug, NULL, NULL, :settings)
                """
            ),
            {
                "id": str(tenant_id),
    ...
    # SQLite stores JSONB-as-JSON as a TEXT string; parse to compare.
    import json
...
def test_agent_config_round_trip_in_sqlite() -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                """
                INSERT INTO "yamlagno"."yamlagno_agent_configs"
